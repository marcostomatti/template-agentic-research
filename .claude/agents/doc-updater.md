...
You edit tracked prose in a bun-workspaces monorepo. There is no codemap
generator, no `docs/CODEMAPS/` tree and no doc build here: every tracked
document is hand-written and hand-maintained, so the deliverable is an
edit you measured, never a file you regenerated.

Read the `AGENTS.md` map that owns the file you are editing, then the
`context/` page for its subject. The root map lists five pages, and
`packages/service` and `packages/web` each keep their own set.
...
- **Wrap width is a per-FILE measurement, never a house number.** No
  ...
  (measured today, per-file caps run from 71 to 78 cols). Take the
  file's own figure before editing it, and count CHARACTERS: both `awk`
  and `wc -L` count BYTES here, so a line carrying a non-ASCII dash
  reads two columns wider than it is (probed, 9 against 7 on one such
  line).
...
- **Never run `lint:fix` over hand-wrapped prose.** The
  ...
  hand-wrapped comment or TSDoc block into a 95-130 char one-liner and
  nothing reports the reflow. Re-measure any file you had wrapped by
  hand after an autofix has touched it.
...
- Repo-root markdown (`AGENTS.md`, `CLAUDE.md`, `context/*.md`) and
  markdown under `tools/` are in the `eslint .` target set, so
...
- Everything under `packages/**` and `.claude/**` answers that ignored
  shape, the root config ignoring `packages/**` and each package's own
  script being a fixed pathspec that names no doc directory. There the
  whole automated reading is `bun run gate:control-bytes`, whose
  `--staged` mode is the non-vacuous half (scanned equals staged), and
  every prose law above is hand-run.
...
and owns the TSDoc, TypeDoc and OpenAPI rules;
`.claude/skills/progress-hygiene/SKILL.md` governs what leaves
`progress.txt` and what is deleted. `stale-prose-sweep-on-commit`,
