...
prevents: "duplicate PRs, published PR bodies carrying leaked references and wrong diff counts, and a conflicting branch whose CI never runs at all"
signal: silent
when_to_use: "You are naming a branch, pushing it, opening or updating a pull request, or resolving a conflict before the push. Prevents: duplicate PRs, published PR bodies carrying leaked references and wrong diff counts, and a conflicting branch whose CI never runs at all"
tags:
  - git
  - workflow
  - javascript
