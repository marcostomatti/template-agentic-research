...
BOTH OpenAPI runtime halves sit in `dependencies` instead, and not just `swagger-ui-express`: `index.ts` imports `generateOpenApiDocument` at boot to build the document it serves, so `@asteasolutions/zod-to-openapi` is a runtime dependency too.
