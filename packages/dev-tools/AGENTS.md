...
| `src/vite/endpoint.ts` | Routing and the route handlers ALONE: `GET /__devtools/status` (git, persistence, the `repo` slug), `GET /templates` (the parsed issue forms, read per request, `[]` where there is no directory), `POST /report` (body, log). One `ROUTE_METHODS` map is both the path match and the method each path answers. Its request plumbing lives in `http.ts`; its cases live in `plugin.test.ts`, since the middleware is reached only through `assembleDevTools`. |
