...
| `src/core/bus.ts` | Pub/sub: four topics (`error`, `route`, `artefact`, `open-item`). The first three carry `unknown` payloads the app's producers settle; `open-item` carries `{featureId, itemId}` and is typed, being the one topic the widget acts on. `src/core/globalCapture.ts` is the only producer inside the package; every other payload is the app's. `last(topic)` answers the newest payload; `recent(topic, n)` answers up to `n` of them newest-first out of a ring capped at 20 per topic, empty for an untouched topic and for `n <= 0`. Each publish REPLACES the ring and `recent` slices a copy, so an array a caller holds neither grows nor writes back. |
| `src/core/globalCapture.ts` | The package's own producer on the bus: `installGlobalCapture(bus?)` adds a window `error` listener and a window `unhandledrejection` listener, each publishing one `{message, topFrame, href, at}` payload on the `error` topic, and answers a disposer that removes BOTH. Defaults to `devtoolsBus`; a caller passes its own bus to keep payloads out of the shared ring. Needs no React tree, which is why this half is the package's and the app's boundary catches only what renders under it. No `preventDefault` — the platform still logs the failure. No `typeof window` guard: the call site is the dev-only bridge, and the refusal leg would be a branch no jsdom case could red. Installing twice installs twice, by design. |
...
| `src/core/Shell.tsx` | Top: owns state (`status`, `settings`), renders trigger/menu/surface/about. The one subscriber in the package: `open-item` on `host.bus`, taken on mount and dropped on dispose (the effect returns the bus's own disposer). A resolved item goes through the SAME `chooseItem` a menu row calls; a `null` from `openItem.ts` announces "Nothing to open" (`shellRules.ts`'s `DEVTOOLS_NOTHING_TO_OPEN`) in the `role="status"` region and opens nothing. A publish does not dismiss an open menu — only `Menu.tsx`'s own `dismiss` returns focus to the trigger. |
...
| `src/index.ts` | Browser entry: exports types, `mountDevTools`, `devtoolsBus`, and `shouldMountDevTools` with its `DevToolsMountDecision` type — the app needs the mount DECISION as a value, because `mountDevTools` answers a plain `DevToolsDisposer` whether it mounted or refused and spec decision 7 keeps that return unchanged. Its own cases are `src/index.test.ts`, collected by the jsdom project through the `src/*.test.ts` pattern. |

## Two vitest projects

**jsdom (`src/*.test.ts` + `src/{core,features}/**/*.test.ts`)**: Browser layer; `.ts`
