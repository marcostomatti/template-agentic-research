...
/**
 * The journal and ledger readers `schema.live.test.ts` holds this
 * database to, declared in `scripts/migration-ledger.ts` and re-exported
 * here rather than copied, so the live suite and any script reading a
 * deployment's ledger name a row one way. The ledger reader takes the
 * `Pool` `createLivePool` answers.
 */
export {
