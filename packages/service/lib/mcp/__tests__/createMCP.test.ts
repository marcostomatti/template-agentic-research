...
describe('createMCP', () => {
  ...
  it('calls wireHttpTransport when resolveTransport returns "http"', async () => {
    const { resolveTransport, wireHttpTransport, wireStdioTransport } = await import('../transport.js');
  ...
  it('starts health server after transport is connected in HTTP mode', async () => {
    const { startHealthServer } = await import('../health.js');
    const { resolveTransport, wireHttpTransport } = await import('../transport.js');
    vi.mocked(resolveTransport).mockReturnValueOnce('http');
  ...
  it('calls process.exit(1) when start() throws', async () => {
    const { wireHttpTransport } = await import('../transport.js');
