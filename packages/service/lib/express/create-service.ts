...
export async function createService(config: ServiceConfig): Promise<ServiceHandle> {
  ...
  // 5. Error handler — always last. The rich handler from lib/errors: maps
  // ZodError → 422 VALIDATION_ERROR, AppError subclasses → their own status
  // and toJSON() body, anything else → opaque 500.
