...
What is still deferred is the one row above that carries no `landed`
mark, and the reason has changed for it. `verify-external.sh` is
verification tooling for the EXTERNAL mode, which nothing here
exercises yet. It is q16b's, whose remainder landed `check-doc-links.ts`
first and then `test-stack.sh`, the disposable stack that verification
is read against rather than anything live.
