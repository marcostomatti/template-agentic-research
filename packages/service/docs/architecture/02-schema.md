...
small CLI (`scripts/approve.ts`) and, since q13, through
`POST /entities/:id/approve-research` beside it, with a UI over either
still to come, and corrected by an operator at a psql prompt when
something has gone wrong. A branch governs the writes of the one thing
it sits in and leaves every other writer to its own habits — and the
writes it does not cover are exactly the ones nothing was watching.
