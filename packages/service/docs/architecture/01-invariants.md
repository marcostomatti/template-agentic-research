...
which is why that one names no artifact at all. The acyclicity row came
after those, drawn from `.specs/q18-runaway-control.md`, which measured
...
standing beside one. The compose row came next, drawn from
`.specs/q16a-compose-n8n.md`, and it is the first row here whose
subject is the STRUCTURE of a file nothing in the verification order
compiles or runs. The three de-origination rows already read a `.yml`
and a `.sh`, but they read them for names; `lint` and `check-types`
open neither at all. So the literals `scripts/activate-workflows.sh`
transcribes out of the compose file next door were held in step by
nobody, and what that spec asks for is the reader and the case that
hold them. The credential row is the newest and comes from that same
spec, whose third measured finding is that every credential-bearing
node across the six workflows names one of exactly two FIXED ids.
That one is not a structural row: both its sides are files the gates
already read, a generated tree and a TypeScript roster. What nothing
held was the agreement between them, and a disagreement there is the
quietest failure this port has — the import, the activation and the
publish all report success over it.
...
| The n8n compose service answers the literals `scripts/activate-workflows.sh` transcribes out of the compose file next door | `tests/invariants/compose-service.test.ts`, over the per-service reader in `tests/invariants/compose-service.ts`, which parses `docker-compose.yml` into which services exist, which profiles gate each one, what image each pins, what container name each declares and which environment keys each sets. Four literals are held: the service key and the profile in the compose command the script's refusal tells an operator to run, the container name it defaults `AR_N8N_CONTAINER` to, the image version every measurement behind this port was taken against, and none of the three settings that move the sqlite the script opens by literal path. The script's own literals are READ out of the script rather than transcribed beside the compose file's, so a rename on one side alone is what reddens, and the forbidden-setting roster names the path segment each setting moves and is held to the path the script really opens. The reader folds compose's mapping and list environment spellings into one key set — that half of the rule is an absence check, which a list entry handed back whole would pass over — and refuses to report a result at all when it read no service. Every assertion is paired with a plant made by changing one thing in the real document and running it back through the same reader, the no-op plant asserted equal to the real read first. What no case can see is the RESOLVED configuration: `docker compose config` folds `${VAR:-default}` and pulls an unprofiled service into a profile's set through `depends_on`, and it needs docker installed, which the isolated suite does not get to assume | q16a | Implemented |
| Every credential a built workflow binds is one the local bootstrap creates, and every credential it creates is one some workflow binds | `tests/invariants/workflow-credentials.test.ts`, holding the `(type, id, name)` triples the reader in `tests/invariants/workflow-credentials.ts` collects out of `workflows/dist/` set-equal against `CREDENTIAL_ROSTER` in `scripts/n8n-credentials.ts`. n8n resolves a binding BY ID, so a triple on one side alone is a node bound to nothing while the import, the activation and the publish each report success, and the failure arrives at the first node that opens a socket on an operator's machine. The reader takes the type from the `credentials` mapping's KEY and the id and name from the value under it; it refuses a walk handed no workflow, so an unbuilt tree cannot report as six canvases that lost their bindings; and it refuses a binding short of a string `id` or `name` rather than dropping one, because a set comparison is blind to a single node among forty — measured, by deleting one artifact's bindings outright and watching the equality stay green with only the provenance case red. Both sides are asserted non-empty above the equality, two empty sets being set-equal; the difference is reported as named triples in both directions rather than as counts, the artifact-side direction first because it is the one that costs a run, since a rename moves an entry out of each side at once and leaves both counts where they were; and each direction is planted at the document its own side comes from — a copy of the built tree carrying one extra node bound to a credential nobody declares, read back through the same walk, and a copy of the roster array carrying an entry no canvas binds — with the no-op plant asserted equal to the real read first. What no case can see is whether either credential ever landed: n8n encrypts credential data with a salt, so neither a stored row's presence nor its timestamp witnesses a usable one | q16a | Implemented |
| The scratch stack shares no container name, published port or volume with the dev stack and its Postgres listens on the port it publishes | `tests/invariants/scratch-stack.test.ts`, holding `docker-compose.scratch.yml` against `docker-compose.yml` through the reader in `tests/invariants/scratch-stack.ts`, which parses both with `parseDocument` and reads the merge TAG off each list node. A plain `parse` drops `!override` and `!reset` and answers the list underneath, which is the list an untagged overlay holds, and an untagged `ports` or `volumes` list in an overlay is APPENDED to the base's — so a tag-blind reader passes exactly the overlay that still publishes the dev ports, and a case shows a plain `parse` answering the real overlay and a copy with every tag stripped identically while this reader tells the two apart. Over what it reads, the reader models the merge rules the overlay relies on — a scalar or an environment key replaced, a list under `!override` replaced, a list under `!reset` left empty whatever it carries, an untagged list appended — and refuses to report a result at all when it read no service. The cases hold what those rules leave: both containers under their scratch names and neither under a name the base fixes; Postgres creating `ar_scratch` and its healthcheck probing that database; every port list the base declares replaced under `!override`, leaving one binding per service, on `127.0.0.1`, on its own port, and on no port a base service publishes; every volume list emptied under `!reset`, no mount left and no volume declared; and `PGPORT` equal to both halves of Postgres's binding and to its healthcheck's `-p`, with no `command` able to move the listener past it — because `postgresCredentialData` in `scripts/n8n-credentials.ts` keeps a URL's port while it rewrites a loopback host to `postgres`, so a scratch database published on another port than it listens on leaves the `ar-postgres` credential dialling nothing. The overlay pins no image and names no service the base lacks, so it is never used alone. Each rule answers named faults rather than a count, the real files asserting none, and every assertion is paired with a near miss planted in the document with its tags kept — among them the untagged list, `!reset` on a port list, `!override` carrying the dev volume, a `55432:5432` binding, and base-side plants taking a scratch name or port — the no-op plants asserted equal to the real reads first. What no case can see is compose's own merge or a running server: `docker compose config` over both files and a reading off a started container are what show those, and both need docker installed, which the isolated suite does not get to assume | q16b | Implemented |
| `verify-external.sh` reaches no request or statement that changes the deployment it reads | `tests/invariants/verify-external-readonly.test.ts`, over the two readers in `tests/invariants/verify-external-readonly.ts` — one over `read-deployment.ts`'s own `ImportDeclaration`s and the `ExportDeclaration`s of the seven scripts/ modules it imports, so a name reaches only through a declaration and a comment naming one of the five mutating n8n calls is no hit, measured against the real near miss `deployment-verdict.ts` and `n8n-client.ts` already carry in prose; the other a line scan over `verify-external.sh` holding it to no invocation but the one leg line, comments stripped the same way, measured against the real `docker`/`curl`/`bun -e` mentions in its own header — with a planted near miss and a planted true hit beside every assertion | q16b | Implemented |
