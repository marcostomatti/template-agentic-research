...
| `scripts/` | Operator entry points: `seed.ts` applies the `data/` bundle, `approve.ts` lists the rows waiting on a ruling and approves or rejects one. Workflow build, deploy, activation and audit landed in phase 3; `bootstrap.sh` takes a cold checkout to a local n8n holding the six workflows, armed and registered, and the rest of the stack-lifecycle group — the panic stop, the scratch stack, the external verification — and the doc-link check arrive in phase 7. See `scripts/README.md`. |
