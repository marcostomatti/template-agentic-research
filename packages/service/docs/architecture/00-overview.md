...
| `src/lib/` | Pipeline libs, written dual-context so the workflow build can splice one into a Code node body. `schedule.ts` is the first and landed in phase 3, which is what proves the splice over a library this package ships rather than a fixture; the ported wave landed in phase 4 — structured-text, delimited-record and message parsing, untrusted-text neutralization, entity-name validation, near-duplicate hashing, audit lines, chunk preparation, and the gating, scoring and feature mechanisms; phase 5 landed the wave its workflows splice: the parse engine over the two `sources` columns that configure it, the markup selector given to it rather than imported by it, fail, flag and keep as one decision about a source row, the boundary a pushed capture has to clear, the fence that keeps untrusted text evidence rather than instruction, and the version a stored feature vector is comparable under; phase 6 landed two more, one for each workflow that phase added. `digest-assemble.ts` is `ar-digest`'s: everything about a period that is a function of the rows rather than of a model, which is what lets a digest be assembled once and stored, so the four export formats render one text instead of four renderings free to disagree about what the period came to. `research-brief.ts` is `ar-research`'s: the judgement an answer has to pass before it is recorded, which refuses rather than repairs — a refused answer leaves its `research_pool` row unstamped to come round again, and approves nothing, approval being the database's. What is deliberately NOT here is the other bound on a schedule proposal: `ar-research` clamps its proposed gap through `schedule.ts` and then meets the streak cap in the SQL that closes the pass, because that cap reads a counter column the proposing node never sees and has to be decided in the same `UPDATE` that moves the due time — a rule about a schedule lives here only while it is a function of what the caller already holds. Distinct from the framework `lib/`. |
...
| `src/mcp/` | The MCP surface. `src/mcp/tools/registry.ts` holds both halves the way `src/exports/index.ts` and `src/sources/index.ts` each hold their own pair: what a tool IS, and `MCP_TOOLS`, the written-out list of the ones this service exposes — composed a wave at a time from `wave-1.ts`, `wave-2.ts` and `wave-3.ts`, twenty-seven entries, twenty of them reads and seven the mutations the API spec names among its safe ones. A tool arrives by an edit to that literal and never by a file appearing in the directory, which matters more here than in either of those registries: an entry is a capability a client can call without anything in the database naming it first. Every entry carries the input schema its HTTP route already exports rather than a copy of it, and calls the same service function that route calls, so the two protocols cannot drift into answering differently about one act. `server.ts` registers every entry in one loop and is the only module here that starts anything: it calls `createMCP` at module scope, which is why the registry is the half a test can import. The HTTP resource routes are not here: each group holds its own router, elsewhere in this table. |
| `src/http/` | The shared boundary under that surface, one declaration each: the success envelope and the pagination meta derived from the window and the store's count, the slug/id param and pagination query schemas, and the boundary parser whose validation details name a field path and never a submitted value, and the control-byte mask and code-point cut a stored payload is served through before the source failures queue answers with it. The resource groups in this table are its only readers but one: the entry module reads the translation of the two app-wide settings, the allowed cross-origin list and the rate-limit count, into what the framework's middleware takes. See `docs/architecture/08-http-api.md`. |
| `src/domains/` | A domain and its settings, addressed by slug — and the arrangement every other resource group in this table copies: a port, one drizzle implementation of it, the rules as plain functions over the port, and a router. The port is what lets every rule be exercised with no database, so the live suite is left proving only that real Postgres agrees. The settings are the one payload this group validates and does not apply: two members of `DomainSettings` are read by no module here at all, only in workflow SQL — the window a repeat research subject is refused inside, and the cap on consecutive agent reschedules of one topic — each read behind a guarded cast with a fleet default standing behind it where the domain names none. |
...
| `src/findings/` | Scored results, the table a digest is made of, met in its domain and addressed by its own id — and the four-module arrangement plus a fifth, `verdict-service.ts`, because a ruling is a decision of its own and the port's ONE write is what it makes. That write APPENDS a `finding_labels` row and never updates one: the table carries no unique key, so a re-judgement is another row and the sequence of them is the record of an operator changing their mind. The ladder it is checked against is read per request off the owning domain, so a refusal names the accepted set and never the verdict somebody sent. The page order is `compareFindings` from `src/lib/digest-assemble.ts` written out in SQL, which is what keeps the ranking this surface answers and the digest the pipeline assembles one order rather than two free to disagree; and the category a finding is filed under is the `fields.category` member that same library reads, no column linking the two. |
| `src/documents/` | Raw ingested items, one read-only page under `/domains/:slug/documents` — the corpus a domain's findings were drawn from, failed parses included rather than held behind a flag, since a document that would not parse is what somebody debugging an ingest goes looking for. The port declares no write method at all, so read-only is a property of the type rather than a promise the router keeps: the edit that would offer to re-parse one has nothing to call. A stored body is untrusted text, answered through the control-byte mask and the code-point cut `src/http/` above holds, at the one `BODY_CODE_POINT_CAP` the source failures queue cuts at too. |
| `src/entities/` | The registry a domain tracks across its findings, addressed by its own id, and the gate over the research queued against it. Two writers and no third: a patch of the row, and the approval that stamps a `research_pool` candidate. `entity_research` is `ar-research`'s to write and no method here has a member for one to arrive through — the API ratifies, the pass records. A pool row also carries `root_event_id`, the run it traces back to, which no method here writes or answers: a stamp for the cost-attribution join and not a depth counter, nothing this pipeline writes being derived from a research answer. That acyclicity is held rather than assumed, and this group is why the rule is written over statements rather than over modules — the store here legitimately reads `entity_research` and writes `research_pool`, so a rule that kept the two apart by mention would have refused the module it landed beside. A `name` patch recomputes `name_norm` through `normalizeEntityName` in `src/lib/entity-name-norm.ts` and never accepts one, which is the single definition that column's own TSDoc asked for, held where a future workflow writer can splice the same reduction. The two alias rules Postgres cannot state — a row aliased to itself, and one aliased across domains — are held here, both being storable as far as the column is concerned and neither a merge anybody meant. |
| `src/runs/` | What the executor has done: a page of passes, one pass with its model-call ledger, and the spend summary over that ledger. Six modules rather than four, because the summary is a second router — `spend-service.ts` and `spend-routes.ts` beside `service.ts` and `routes.ts`. The aggregation lives in this directory rather than a <!-- doc-links-skip: src/spend/ -- counterfactual path: pattern explains why this directory doesn't exist -->`src/spend/` of its own because it is over `llm_calls`, and `llm_calls` hangs off `runs` and has no directory of its own: a directory holding one read over another table's rows would be named for a question rather than for a subject. Nothing on the port writes, so neither a run nor a ledger row can be opened from here — causing work has one spelling on this API and it is `run-now` setting a schedule column. Spend is a count and two magnitudes and no member of it is currency, `llm_calls` carrying no price, rate or amount column for one to answer from. And `runs.domain_id` is nullable, so a domain-less maintenance tick is an ordinary row the unfiltered page holds. |
| `src/approvals/` | The approval vocabulary, and the one row here that is neither a resource group nor a boundary: no router, no port, no store, and no import at all. It holds the part of a ruling both gates share — the projection either answers with, the closed roster of reasons either may refuse for, and which of the two acts tolerates being repeated — so the `research_pool` gate in `src/entities/` above and the `source_config_proposals` gate in `src/sources/` above cannot drift into answering differently about one decision. `scripts/approve.ts` already ruled on both from one CLI; this is what two HTTP routes in two directories needed in order to agree with it and with each other. |
...
| `scripts/` | Operator entry points: `seed.ts` applies the `data/` bundle, `approve.ts` lists the rows waiting on a ruling and approves or rejects one. Workflow build, deploy, activation and audit landed in phase 3; `bootstrap.sh` takes a cold checkout to a local n8n holding the six workflows, armed and registered, and `panic.sh` is its counterpart, disarming everything the instance `AR_N8N_URL` names has armed through `panic-external.ts` and then stopping this compose project's containers whatever profile gates them. `test-stack.sh` stands a disposable `ar-scratch` copy of that stack up beside the dev one, every compose call naming the project and both compose files, and takes it down again on a read-back of what survived; the env file it writes points a shell's `bootstrap.sh` and `panic.sh` at the copy. `verify-external.sh` completes the stack-lifecycle group: it verifies an external-mode deployment without changing it, running `read-deployment.ts` once per leg as a process of its own and carrying every failure to one verdict line per leg, and exits 2 before any leg where a setting it needs is not exported, 0 only when every leg answered 0, and 1 otherwise. q16b landed `check-doc-links.ts` beside it: run from the repository root as `bun run gate:doc-links`, it holds every path the tracked docs name against the tracked set. See `scripts/README.md`. |
...
| HTTP API | `src/http/`, `src/domains/`, `src/taxonomy/`, `src/personas/`, `src/settings/`, `src/topics/`, `src/connectors/`, `src/subscriptions/`, `src/findings/`, `src/documents/`, `src/entities/`, `src/runs/`, `src/approvals/`, and the HTTP half of `src/sources/` | `docs/architecture/08-http-api.md` |
...
and wave 2's four groups and the five paths wave 3 adds landed against a
document written the same way; the Layout table above names every one of
them, so every path in the column resolves. A reservation there is
discharged by the commit that creates the directory, not by a later
pass.
