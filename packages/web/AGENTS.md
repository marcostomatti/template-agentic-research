...
| `src/components/` | App-local stand-ins for `@ar/ui` components that do not exist yet, the shared list-page skeleton, and the pure `.ts` modules the editor modals share (`editorDraft.ts`). See below. |
