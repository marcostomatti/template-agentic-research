...
| `src/data/` | The data layer seam: `api.ts` (the Vite-replaced selector), `source.ts` (the resolver), `hooks.ts` (one cache hook per accessor), `types.ts` (the redeclared schema vocabulary), and one-line re-exports for every fixture module with importers outside `src/data/fixture/`. See `context/data.md`. |
...
| `src/data/http/` | HTTP implementations: `api.ts` (34 accessor stubs), `client.ts` (session + fetch), `envelope.ts` (response shape), `operator.ts` (derive from sub claim). Every stub besides `fetchOperator` rejects with `NOT_WIRED`. |
