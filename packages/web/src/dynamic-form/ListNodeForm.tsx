 ...
 * The LIST branch of the one mounted form: one row per ITEM.
 *
 * `./NodeForm.tsx` owns the envelope and the switch over a
 * container's type, and this file is the half that switch reaches
 * for a list — split out of it so neither file has to grow past
 * the other's reading. The object half stays there, next to the
 * switch, because it is four lines of `map`; the list half is
 * everything below: `Sortable`, the pair of move controls on every
 * row, and the derivation that makes a drop and a keypress the
 * same report. Nothing crossed the boundary but the code: the
 * props this file takes extend `./NodeForm.tsx`'s own, the
 * narrowing arriving with the def rather than being re-read here.
 *
 ...
 *
 * The runner fact `./NodeForm.tsx` and `./FieldControl.tsx` both
 * record: `@ar/web` runs vitest over `src` in a NODE environment
 * with an include of `*.test.ts`, so a `.tsx` is neither collected
 * nor renderable there. What is left here is composition, one
 * derivation and three reports, and the two measurements that
 * reach them are stated where the mount is — `./NodeForm.tsx`'s
 * header carries the static-render probe's readings, this file
 * being what that probe draws for a list node and its
 * `onReorder` reading being taken off the `Sortable` element
 * below.
 *
 * The `check-types` legs that land in THIS file, all measured
 * from inside `packages/web` at EXIT 2. A leaf def handed to
 * {@link ListNodeForm} is TS2322 at the JSX prop. A reorder index
 * typed as a string reds BOTH codes whichever prop declares it —
 * TS2345 at the call it broke and TS2322 at the prop forwarding
 * the old handler on — so expect one of each; mutating the
 * ARGUMENT instead reds TS2345 alone. Dropping `'down'` from
 * {@link MoveDirection} is TS2322 at the roster literal plus
 * TS2353 at both keyed tables. What none of it reaches is the
 ...
 * `./NodeForm.tsx`'s switch narrowed, handed down beside the
